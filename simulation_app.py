...
<style>
  ...
  /* 속도계 */
  .speedometer {
      background: radial-gradient(circle, #1a1a3e, #0d0d1a);
...
# ── 트랙 생성 ─────────────────────────────────────────────────
def make_track(heights: list, n_points: int = TOTAL_FRAMES) -> tuple:
    """제어점 높이로 부드러운 트랙 생성"""
    n_ctrl = len(heights)
...
# ── 메인 시뮬레이션 플롯 ─────────────────────────────────────
def draw_simulation(mass: float, heights: list, frame_idx: int):
    """현재 프레임에 맞는 Plotly 그래프 반환"""
    x, y   = make_track(heights, TOTAL_FRAMES)
    pe, ke, te, v = compute_energies(mass, y)

    idx    = np.clip(frame_idx, 0, TOTAL_FRAMES - 1)
    car_x  = float(x[idx])
    car_y  = float(y[idx])
    cur_pe = float(pe[idx])
    cur_ke = float(ke[idx])
    ...
    # 트랙 그라데이션 효과 (높이별 색상)
    # 세그먼트를 잘라 색상 매핑
    norm_h = (y - y.min()) / (y.max() - y.min() + 1e-9)
    for i in range(0, len(x) - 1, 3):
        c_val  = norm_h[i]
        r = int(79  + (255 - 79)  * c_val)
        g = int(195 + (100 - 195) * c_val)
        b = int(247 + (50  - 247) * c_val)
        fig.add_trace(go.Scatter(
            x=x[i:i+4], y=y[i:i+4],
            mode="lines",
    ...
    if idx > 0:
        fig.add_trace(go.Scatter(
            x=x[:idx+1], y=y[:idx+1],
            mode="lines",
            line=dict(color="rgba(255,255,255,0.25)", width=7),
            showlegend=False, hoverinfo="skip",
        ), row=1, col=1)
    ...
    # 속도 벡터 화살표
    if cur_v > 0.3 and idx < TOTAL_FRAMES - 2:
        dx = float(x[min(idx+5, TOTAL_FRAMES-1)] - x[idx])
        dy = float(y[min(idx+5, TOTAL_FRAMES-1)] - y[idx])
        norm = np.sqrt(dx**2 + dy**2) + 1e-9
        arrow_scale = min(cur_v * 0.6, 8)
        fig.add_annotation(
            x=car_x + dx/norm * arrow_scale,
            y=car_y + dy/norm * arrow_scale + 1.2,
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines",
        line=dict(color="rgba(255,217,61,0.5)", width=1.5, dash="dot"),
        showlegend=False, hoverinfo="skip",
    ), row=1, col=1)
    ...
    fig.add_annotation(
        ...
        font=dict(color="#ff6b6b", size=11, family="monospace"),
        showarrow=False,
        row=1, col=1,
    )
    ...
    fig.add_trace(go.Bar(
        ...
        text=[f"{val:.0f}J" for val in bar_values],
    ...
    if idx > 0:
        fig.add_trace(go.Scatter(
            x=np.concatenate([x[:idx+1], x[:idx+1][::-1]]),
            y=np.concatenate([pe[:idx+1], np.zeros(idx+1)]),
            fill="toself",
...
with st.sidebar:
    ...
    if preset != "직접 설정":
        ...
        if new_h != st.session_state["heights"]:
            st.session_state["heights"]    = new_h
            st.session_state["frame"]      = 0
            st.session_state["is_running"] = False
            st.session_state["is_finished"]= False
            st.session_state["max_speed_seen"] = 0.0

    if preset == "직접 설정":
        st.markdown("**각 지점 높이 (m)**")
        new_heights = []
        cols_h = st.columns(2)
        labels = ["출발점", "지점 2", "최고점", "지점 4", "지점 5", "도착점"]
    ...
    # 질량
    new_mass = st.slider("🏋️ 카트 질량 (kg)", 10, 200,
                          int(st.session_state["mass"]), 5)
    if new_mass != st.session_state["mass"]:
        st.session_state["mass"]       = float(new_mass)
        st.session_state["frame"]      = 0
        st.session_state["is_running"] = False
        st.session_state["is_finished"]= False
    ...
    st.session_state["anim_speed"] = st.select_slider(
        ...
        format_func=lambda x: {1:"🐢 느리게", 2:"🚶 보통",
                                 3:"🏃 빠르게", 5:"🚀 매우 빠르게",
                                 8:"⚡ 초고속"}[x],
    )
...
# ── 메인 영역 ─────────────────────────────────────────────────
st.markdown('<div class="main-title">🎢 롤러코스터 에너지 탐험</div>',
            unsafe_allow_html=True)
st.markdown(
...
with tab_sim:
    ...
    with ctrl_left:
        ...
        with btn_col3:
            if st.button("⏭ 끝으로", use_container_width=True):
                st.session_state["is_running"]  = False
                st.session_state["frame"]       = TOTAL_FRAMES - 1
                st.session_state["is_finished"] = True
                st.rerun()

        # 수동 이동 슬라이더 (정지 상태에서만 활성)
    ...
    with ctrl_mid:
        ...
        if st.session_state["is_running"]:
            st.markdown('<span class="status-running">🟢 주행 중</span>',
                        unsafe_allow_html=True)
        elif st.session_state["is_finished"]:
            st.markdown('<span class="status-finished">🏁 도착!</span>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<span class="status-stopped">⚪ 정지</span>',
                        unsafe_allow_html=True)

        # 진행률
        progress_val = st.session_state["frame"] / (TOTAL_FRAMES - 1)
        st.progress(progress_val)
        st.caption(f"진행률: {progress_val*100:.1f}%  |  "
    ...
    with ctrl_right:
        ...
        # 현재 에너지 계산 (표시용)
        x_tmp, y_tmp = make_track(st.session_state["heights"], TOTAL_FRAMES)
        pe_tmp, ke_tmp, te_tmp, v_tmp = compute_energies(
            st.session_state["mass"], y_tmp
        )
        f = st.session_state["frame"]
        cur_pe_d = float(pe_tmp[f])
        cur_ke_d = float(ke_tmp[f])
        cur_te_d = float(te_tmp[f])
        cur_v_d  = float(v_tmp[f])
        cur_h_d  = float(y_tmp[f])
...
<div class="energy-bar-wrap">
  <div class="pe-fill" style="width:{pe_w:.1f}%">
    {"PE " + f"{pe_pct:.0f}%" if pe_w > 15 else ""}
  </div>
  <div class="ke-fill" style="width:{ke_w:.1f}%">
    {"KE " + f"{ke_pct:.0f}%" if ke_w > 15 else ""}
  </div>
</div>
""", unsafe_allow_html=True)
        ...
        # 최고 속도 추적
        if cur_v_d > st.session_state["max_speed_seen"]:
            st.session_state["max_speed_seen"] = cur_v_d
...
""", unsafe_allow_html=True)
    ...
    # 높이/속도에 따른 동적 힌트
    h_ratio = stats["height"] / (max(st.session_state["heights"]) + 1e-9)
    if h_ratio > 0.8:
        hint_msg = "🏔️ **높은 곳!** 위치에너지가 최대입니다. 속도가 느리죠? **PE↑, KE↓**"
        hint_color = "#4ecdc4"
    elif h_ratio < 0.15:
        hint_msg = "⚡ **낮은 곳!** 위치에너지가 운동에너지로 전환되어 속도가 최대예요! **PE↓, KE↑**"
        hint_color = "#ff6b6b"
    elif stats["speed"] > stats["height"]:
        hint_msg = "🚀 **가속 중!** 높이가 낮아지면서 운동에너지가 증가하고 있어요."
        hint_color = "#f7971e"
    else:
        hint_msg = "⚖️ **에너지 전환 중!** PE + KE = 일정 — 역학적 에너지 보존 법칙이 작동 중!"
        hint_color = "#a29bfe"

    hint_placeholder.markdown(f"""
<div style="background:rgba(255,255,255,0.04); border-left:4px solid {hint_color};
            border-radius:8px; padding:12px 16px; margin-top:8px;">
  <span style="color:{hint_color};">{hint_msg}</span>
</div>
""", unsafe_allow_html=True)

    # ── 애니메이션 루프 ───────────────────────────────────────
    if st.session_state["is_running"]:
        speed_delay_map = {1: 0.12, 2: 0.07, 3: 0.045, 5: 0.025, 8: 0.01}
        delay = speed_delay_map.get(st.session_state["anim_speed"], 0.07)
