...
def draw_simulation(mass: float, heights: list, frame_idx: int):
    ...
    if idx > 0:
        fig.add_trace(go.Scatter(
            x=x[:idx+1], y=y[:idx+1],
            mode="lines",
            line=dict(color="rgba(255,255,255,0.22)", width=8),
            showlegend=False, hoverinfo="skip",
        ), row=1, col=1)
    ...
    # 속도 벡터 화살표
    if cur_v > 0.3 and idx < TOTAL_FRAMES - 5:
        look = min(idx + 6, TOTAL_FRAMES - 1)
        dx   = float(x[look] - x[idx])
        dy   = float(y[look] - y[idx])
        nm   = np.sqrt(dx**2 + dy**2) + 1e-9
        sc   = min(cur_v * 0.55, 8.0)
        fig.add_annotation(
            x=car_x + dx/nm*sc, y=car_y + dy/nm*sc + 1.2,
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines",
        line=dict(color="rgba(255,217,61,0.45)", width=1.5, dash="dot"),
        showlegend=False, hoverinfo="skip",
    ), row=1, col=1)
    fig.add_annotation(
        x=car_x, y=car_y / 2,
...
with tab_sim:
    ...
    with st.container():
        ...
        with ctrl_right:
            ...
            pe_label = f"PE {pe_pct:.0f}%" if pe_w > 15 else ""
            ke_label = f"KE {ke_pct:.0f}%" if ke_w > 15 else ""
            st.markdown(
                f"""
<div class="energy-bar-wrap">
  <div class="pe-fill" style="width:{pe_w:.1f}%">{pe_label}</div>
  <div class="ke-fill" style="width:{ke_w:.1f}%">{ke_label}</div>
</div>
""",
                unsafe_allow_html=True,
            )
...
<div class="hint-box" style="background:rgba(255,255,255,0.04);
  ...
  <span style="color:{hint_color}; font-size:15px;">{hint_msg}</span>
</div>
""",
        unsafe_allow_html=True,
    )
...
위쪽 <b>⚡ 에너지 시뮬레이션</b> 탭에서 <b>▶ 출발!</b> 버튼을 눌러<br>
...
""", unsafe_allow_html=True)
    ...
    # ── 메시지 전송 후 API 호출 ───────────────────────────────
    # (빠른 질문 or 폼 제출로 messages에 user 메시지가 추가된 경우)
    last_role = (
    ...
    if last_role == "user":
        if not st.session_state["api_key"]:
            ...
            # 현재 시뮬레이션 컨텍스트 첨부
            x_c, y_c         = make_track(st.session_state["heights"], TOTAL_FRAMES)
            pe_
